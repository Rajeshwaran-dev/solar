...
import { Menu, Search, Bell, ChevronDown, LogOut, UserCheck } from "lucide-react";
...
export default function AdminHeader({ onMenuClick, title }) {
  ...
  return (
    <header className="shrink-0 flex h-[72px] items-center justify-between gap-4 border-b border-ink-900/8 bg-white/90 px-5 backdrop-blur sm:px-8">
      ...
      <div className="flex items-center gap-2">
        <div className="relative">
          <button onClick={() => { setNotifOpen((v) => !v); setProfileOpen(false); }} className="relative flex h-10 w-10 items-center justify-center rounded-xl text-ink-900 hover:bg-sand-100">
        ...
        <div className="relative">
          <button onClick={() => { setProfileOpen((v) => !v); setNotifOpen(false); }} className="flex items-center gap-2.5 rounded-xl px-2 py-1.5 hover:bg-sand-100">
          ...
          {profileOpen && (
            <div className="absolute right-0 top-full mt-2 w-64 rounded-2xl border border-ink-900/8 bg-white p-2.5 shadow-lift">
              <div className="border-b border-ink-900/8 px-3 py-2">
                <p className="text-xs font-semibold text-ink-900">Sol Admin</p>
                <p className="text-[11px] font-mono text-ink-900/45">admin@solgreen.in</p>
              </div>
