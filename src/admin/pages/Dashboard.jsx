        ...
        <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-5">
          {topProducts.map((p) => (
            <div key={p.id} className="rounded-2xl border border-ink-900/6 p-3">
              <div className="flex aspect-square items-center justify-center rounded-xl bg-sand-100">
                <ProductArt icon={p.icon} tint={p.tint} uid={`top-${p.id}`} className="h-16 w-16" />
              </div>
